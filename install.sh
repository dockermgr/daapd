...
DAAPDSERVER_PORT="${DAAPDSERVER_PORT:-3689}"
DAAPDSERVER_HOST="${DAAPDSERVER_HOST:-$(hostname -f 2>/dev/null)}"
REPO="${DOCKERMGRREPO:-https://github.com/dockermgr}/$APPNAME"
REPO_BRANCH="${GIT_REPO_BRANCH:-main}"
DAAPDSERVER_TIMEZONE="${TZ:-${TIMEZONE:-America/New_York}}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
if docker ps -a | grep -qs "$APPNAME"; then
  printf_blue "Service is available at: http://$DAAPDSERVER_HOST:$DAAPDSERVER_PORT"
else
  false
fi
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
